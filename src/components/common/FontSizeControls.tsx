...
const FONT_SIZE_CHANGE_EVENT = 'blog-font-size-change';

const getStoredFontSize = () => {
  const savedFontSize = localStorage.getItem('blog-font-size');
  const size = savedFontSize ? Number.parseInt(savedFontSize, 10) : 16;

  return Number.isFinite(size) ? Math.max(12, Math.min(24, size)) : 16;
};
